         ...
         if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
          {
           ...
           if ( !is_array($PosArray) ) { $Value = $PosArray; $PosArray = array(); $PosArray[0] = $Value; }
           $LastGoodY = NULL; $LastGoodX = NULL; $LastX; $LastY = 1;
           foreach($PosArray as $Key => $Y)
            {
             if ( $DisplayValues ) 
           ...
           foreach($PosArray as $Key => $Y)
            {
             ...
             if ( $Y != VOID && $LastX != NULL && $LastY != NULL )
              {
               if ( empty($Points) ) {$Points = array(); $Points[] = $LastX; $Points[] = $YZero; }
               $Points[] = $LastX; $Points[] = $LastY;
               $Points[] = $X; $Points[] = $LastY;
               $Points[] = $X; $Points[] = $Y;
              }
           ...
           foreach($PosArray as $Key => $X)
            {
             ...
             if ( $X != VOID && $LastX != NULL && $LastY != NULL )
              {
               if ( empty($Points) ) { $Points = array(); $Points[] = $YZero; $Points[] = $LastY; }
               $Points[] = $LastX; $Points[] = $LastY;
               $Points[] = $LastX; $Points[] = $Y;
               $Points[] = $X; $Points[] = $Y;
              }
