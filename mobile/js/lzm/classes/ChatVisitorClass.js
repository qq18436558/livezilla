...
ChatVisitorClass.prototype.LoadMapSource = function (_mapBody) {
    ...
    var geoTrackingUrl = '',myServerAddress='',gtKey = DataEngine.getConfigValue('gl_pr_ngl', true);
    var host = DataEngine.getConfigValue('gl_host_ngl', false);

    gtKey = (gtKey !== null && d(gtKey.value)) ? lz_global_base64_decode(gtKey.value) : '';

    if (host != 0)
    {
        myServerAddress = DataEngine.getServerUrl('', DataEngine.serverProtocol);
        geoTrackingUrl = DataEngine.getServerUrl('map.php', DataEngine.serverProtocol);
    }
    else
    {
        myServerAddress = 'https://ssl.livezilla.net';
        geoTrackingUrl = 'https://ssl.livezilla.net/geo/map/index.php?web=1&mv=3&pvc=' + lzm_commonConfig.lz_version;
    }
...
ChatVisitorClass.prototype.EditVisitorDetails = function(visitorId,field,elementId){
    ...
    for (var i=0; i<DataEngine.inputList.idList.length; i++)
    {
        ...
        {
            value = lzm_commonTools.escapeHtml(DataEngine.inputList.getInputValueFromVisitor(myCustomInput.id,visitor,null,true));
